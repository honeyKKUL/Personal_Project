...
    // 초기 변환 설정 (CSS에서 translate(-50%, -50%)로 중앙 정렬된 상태를 시작점으로 설정)
    const randomRotation = Math.floor(Math.random() * 360); 
    effect.style.transform = `translate(-50%, -50%) rotate(${randomRotation}deg) scale(0.1)`;

    body.appendChild(effect);
    
    // requestAnimationFrame을 사용하여 렌더링 직후 애니메이션 클래스 추가
    requestAnimationFrame(() => {
        // CSS의 .hit-effect.animate에 transition이 적용되어 애니메이션이 발동됨
